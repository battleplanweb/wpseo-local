...
if ( ! class_exists( 'WPSEO_Local_Admin_API_Keys' ) ) {
	...
	class WPSEO_Local_Admin_API_Keys {
		...
		public function tab_content() {
			...
			echo '<p>' . sprintf( esc_html__( 'For more information on how to create and set your Google Maps API key, %1$scheck our Yoast help center%2$s.', 'yoast-local-seo' ), '<a href="' . esc_url( WPSEO_Shortlinker::get( 'https://yoa.st/gm-api-browser-key' ) ) . '" target="_blank">', '</a>' ) . '</p>';
			...
			if ( empty( $api_key ) && ! empty( $browser_api_key ) ) {
				...
				echo '<p>';
				/* translators: %1$s extends to the anchor opening tag '<a href="https://yoa.st/gm-api-browser-key" target="_blank">', %2$s closes that tag. */
				printf( esc_html__( 'A Google Maps browser key is required to show Google Maps and make use of the Store Locator. For more information on how to create and set your Google Maps browser key, %1$scheck our Yoast help center%2$s.', 'yoast-local-seo' ), '<a href="' . esc_url( WPSEO_Shortlinker::get( 'https://yoa.st/gm-api-browser-key' ) ) . '" target="_blank">', '</a>' );
				echo '</p>';
