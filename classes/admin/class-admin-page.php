	...
	class WPSEO_Local_Admin_Page {
		...
		private static function create_tabs() {
			...
			foreach ( self::$tabs as $slug => $titles ) {
				...
				echo '<a class="nav-tab" id="' . esc_attr( $slug ) . '-tab" href="#top#' . esc_attr( $slug ) . '">' . esc_html( $titles['tab_title'] ) . '</a>';
			}
			echo '</h2>';
		}
		...
		public static function section_before( $id = '', $style = '', $section_class = '' ) {
			echo '<div' . ( isset( $id ) ? ' id="' . esc_attr( $id ) . '"' : '' ) . '' . ( ! empty( $style ) ? ' style="' . esc_attr( $style ) . '"' : '' ) . '' . ( ! empty( $section_class ) ? ' class="' . esc_attr( $section_class ) . '"' : '' ) . '>';
		}
